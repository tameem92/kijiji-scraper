...
function parseHTML(html: string): AdInfo | null {
  ...
  if (
    json.length === 0 ||
    Object.keys((adData = JSON.parse(json))).length === 0 ||
    !adData.hasOwnProperty("config") ||
    !adData.config.hasOwnProperty("adInfo") ||
  ...
  let categoryID = "";
  if (adData.viewItemPage && adData.viewItemPage.viewItemData) {
    categoryID = adData.viewItemPage.viewItemData.categoryId.toString();
  }
  ...
  info.image = (adData.adInfo.sharingImageUrl || "").replace(
    IMG_REGEX,
    "/$_57.JPG"
  );
  ...
  (adData.VIP.adAttributes || []).forEach((a: any) => {
    if (
      typeof a.machineKey === "string" &&
      typeof a.machineValue === "string"
    ) {
